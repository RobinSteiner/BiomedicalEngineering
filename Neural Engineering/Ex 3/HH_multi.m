...
% Stimulus parameters
mode = 'iClamp'; % Stimulus mode, 'iClamp' or 'extracellular'
...
I = 100; % Stimulus amplitude, in pA ('iClamp') or uA ('extracellular' 'point') or V ('extracellular' 'FEM')
    ...
    case 'extracellular' % Compute effect of extracellular electric fields on stick
        ...
        switch elecType % Compute extracellular potentials depending on electrode type
            case 'point'
                ...
                potentials = 1e-3*(rhoE*I)./(4*pi*compDist); % in 1e-3*(Ohm*cm*uA)/cm==mV
            case 'disk'
                ...
                potentials = 1e-3*(2*rhoE*I)/(4*rDisk*pi) * asin((2*rDisk)./(sqrt((r-rDisk).^2+z.^2)+sqrt((r+rDisk).^2+z.^2))); % in 1e-3*(Ohm*cm*uA)/cm==mV
        end
