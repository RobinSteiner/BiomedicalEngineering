...
def detect_spiking(vMat):
    """
    Detects if there is spiking activity in the voltage matrix.
    Returns 1 if there is spiking, 0 otherwise.
    """
...
def HH_multi(cellX):
    ...
    elecType = 'point'  # Electrode type, 'point' or 'disk', for 'extracellular' only
    dDisk = 50  # Electrode diameter, in um, for 'disk' only
    rhoE = 300  # Extracellular resistivity, in Ohm*cm, for 'point and 'disk' only
    I = -5  # Stimulus amplitude, in pA ('iClamp') or uA ('extracellular')
    ...
    # Plot flags
    plotV = 0  # Membrane voltage over time
    ...
    matInvDiag = np.concatenate(([1 + (tDt / C) * (1 / R)], np.ones(nComp - 2) + (tDt / C) * (2 / R),
                                 [1 + (tDt / C) * (1 / R)]))  # in (ms/uF)*(1/kOhm)==1/V
    ...
    if mode == 'iClamp':  # Simple current conversion
        ...
        if elecType == 'point':
            ...
            potentials = 1e-3 * (2.0 * rhoE * I) / (4.0 * np.pi * rDisk) * np.arcsin((2 * rDisk) / (
                        np.sqrt((r - rDisk) ** 2 + z ** 2) + np.sqrt(
                    (r + rDisk) ** 2 + z ** 2)))  # in 1e-3*(Ohm*cm*uA)/cm==mV
    ...
    for t in range(0, timeSteps - 1):
        ...
        if solver == 'FE':
            ...
            # Update gating variables with new v
            mMat[:, t + 1] = (mVecT + tDt * alphaM(vMat[:, t + 1], kTemp)) / (
                        1 + tDt * (alphaM(vMat[:, t + 1], kTemp) + betaM(vMat[:, t + 1], kTemp)))
            hMat[:, t + 1] = (hVecT + tDt * alphaH(vMat[:, t + 1], kTemp)) / (
                        1 + tDt * (alphaH(vMat[:, t + 1], kTemp) + betaH(vMat[:, t + 1], kTemp)))
            nMat[:, t + 1] = (nVecT + tDt * alphaN(vMat[:, t + 1], kTemp)) / (
                        1 + tDt * (alphaN(vMat[:, t + 1], kTemp) + betaN(vMat[:, t + 1], kTemp)))
        else:
